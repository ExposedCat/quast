...
function storeBackup(game: Game, player: Player, units: Unit[]) {
	...
	fs.writeFileSync('./saved/map.txt', game.stringContent(false, player))
	fs.writeFileSync('./saved/units.json', JSON.stringify(units))
	fs.writeFileSync('./saved/player.json', JSON.stringify(player))
	fs.writeFileSync(
		'./saved/game-data.json',
		JSON.stringify({
			info: game.info
		})
	)
...
function restoreGameData() {
	...
	try {
		const gameData = JSON.parse(
			fs.readFileSync('./saved/game-data.json', 'utf-8')
		)
		info = gameData.info
	} catch (e) {}
	return { map, info }
...
export { reDraw, storeBackup, restoreUnits, restorePlayer, restoreGameData }
